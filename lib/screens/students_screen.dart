...
import 'package:labassistant/services/screen_capture_launcher.dart';
...
class _StudentDashboardState extends State<StudentDashboard> with TickerProviderStateMixin, WidgetsBindingObserver {
  ...
  bool _monitoringEnabled = false; // Flag to control when to start monitoring
  ...
  void initState() {
    ...
    // Add the observer for lifecycle changes
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      _checkOnlineStatus();
      _checkIfStudentIsLocked();
      _initializeSocket();
      _loadSubjects();
      
      // Auto-launch screen capture agent for monitoring
      _launchScreenCaptureAgent();
  ...
  // Add the lifecycle state change detection
  @override
  void didChangeAppLifecycleState(AppLifecycleState state) {
    super.didChangeAppLifecycleState(state);
    
    // Only monitor tab switches after monitoring is enabled
    if (!_monitoringEnabled) {
      print('⏸️ Tab switch monitoring not yet enabled, ignoring lifecycle change');
      return;
    }
    
    // Detect when app goes to background (tab switch, minimize, etc.)
  ...
  void _handleTabSwitch() {
    // Don't count tab switches if already locked or monitoring not enabled
    if (_isLocked || !_monitoringEnabled) return;
    
    _tabSwitchCount++;
    print('Tab switch detected in student dashboard. Count: $_tabSwitchCount');
    
    if (_tabSwitchCount == 1) {
      // Show warning for first tab switch
      _showTabSwitchWarning();
    } else if (_tabSwitchCount >= 2) {
      // Lock account on second tab switch
      _lockStudentAndLogout();
    }
  }
      ...
      builder: (context) => AlertDialog(
        ...
        content: Column(
          ...
          children: [
            ...
            Container(
              ...
              child: const Row(
                children: [
                  ...
                  Expanded(
                    child: Text(
                      'Next time you switch tabs, your account will be LOCKED!',
                      style: TextStyle(
                        fontSize: 14,
                        fontWeight: FontWeight.w600,
            ...
            const SizedBox(height: 12),
            const Text(
              'Please stay focused on your work and do not switch tabs or minimize the application.',
              style: TextStyle(
                fontSize: 14,
  ...
  Future<void> _lockStudentAndLogout() async {
    setState(() {
      _isLocked = true;
    });

    try {
      // Update student lock status in database
      if (_apiService != null && _authService?.user != null) {
        print('🔒 Attempting to lock student ${_authService!.user!.enrollNumber} with $_tabSwitchCount tab switches');
        await _apiService!.lockStudent(_authService!.user!.enrollNumber, _tabSwitchCount);
        print('✅ Student lock API call completed successfully');
      } else {
        print('❌ Cannot lock student - API service or user is null');
        print('  - API service: ${_apiService != null ? 'Available' : 'NULL'}');
        print('  - Auth service: ${_authService != null ? 'Available' : 'NULL'}');
        print('  - User: ${_authService?.user != null ? 'Available' : 'NULL'}');
      }
    } catch (e) {
      print('❌ Error locking student: $e');
      print('❌ Stack trace: ${StackTrace.current}');
    }

    // Show locked message and logout
    if (mounted) {
      showDialog(
        context: context,
        barrierDismissible: false,
        builder: (context) => AlertDialog(
          ...
          title: Row(
            children: [
              Icon(Icons.lock_rounded, color: Colors.red[600], size: 28),
              const SizedBox(width: 12),
              const Text(
                'Account Locked',
                style: TextStyle(
          ...
          content: const Text(
            'You have switched tabs 2 times. Your account has been locked for security reasons. Please contact your administrator to unlock your account.',
  ...
  Future<void> _checkIfStudentIsLocked() async {
    try {
      if (_apiService != null && _authService?.user != null) {
        final lockStatus = await _apiService!.getStudentLockStatus(_authService!.user!.enrollNumber);
        final isLocked = lockStatus['isLocked'] ?? false;
        final tabSwitchCount = lockStatus['tabSwitchCount'] ?? 0;
        
        print('📊 Student lock status check:');
        print('   - Is Locked: $isLocked');
        print('   - Tab Switch Count from DB: $tabSwitchCount');
        
        if (mounted) {
          setState(() {
            _isLocked = isLocked;
            _tabSwitchCount = tabSwitchCount; // Initialize from database
          });
          
          print('✅ Initialized tab switch count to: $_tabSwitchCount');
          
          if (isLocked) {
            _showLockedMessage();
          } else {
            // Enable monitoring after a 3-second delay to avoid false positives during navigation
            Future.delayed(const Duration(seconds: 3), () {
              if (mounted && !_isLocked) {
                setState(() {
                  _monitoringEnabled = true;
                });
                print('✅ Tab switch monitoring ENABLED after delay');
              }
            });
          }
        }
      ...
      print('Error checking if student is locked: $e');
    }
  }
  ...
  void _showLockedMessage() {
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: const Text(
          'Your account has been locked due to tab switching violations. Please contact your administrator to unlock your account.',
          style: TextStyle(fontSize: 16, color: Color(0xFF64748B)),
        ),
        actions: [
          ElevatedButton(
  ...
  Future<void> _launchScreenCaptureAgent() async {
    try {
      print('\n🎬 AUTO-LAUNCHING SCREEN CAPTURE AGENT');
      print('👤 Student: ${_authService?.user?.name}');
      
      // Launch the agent silently in the background
      await ScreenCaptureLauncher.launchAgentSilently();
      
      // Get status for logging
      final status = await ScreenCaptureLauncher.getAgentStatus();
      print('📊 Agent Status: $status');
      
      if (status['isRunning'] == true) {
        print('✅ Screen capture agent is running and ready for monitoring');
      } else if (status['executableFound'] == false) {
        print('⚠️  Screen capture agent not found - monitoring unavailable');
        print('💡 Please install ScreenCaptureAgent.exe to enable screen monitoring');
      } else {
        print('⚠️  Screen capture agent may require UAC approval');
      }
  ...
  void _showAdminShutdownDialog() {
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: const Text(
          'Admin has logged out. You will be automatically logged out and redirected to the login screen.',
          style: TextStyle(fontSize: 16, color: Color(0xFF64748B)),
        ),
        actions: [
          ElevatedButton(
...
Future<void> _refreshCompletionStatus() async {
  try {
    if (selectedSubject != null && _apiService != null) {
      ...
      if (mounted) {
        setState(() {
          // This setState will trigger a rebuild with the updated completedExerciseIds
    ...
    if (mounted) {
      _showErrorSnackBar('Error refreshing completion status: $e');
    }
  }
}
