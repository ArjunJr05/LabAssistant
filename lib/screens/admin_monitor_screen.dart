...
class _AdminMonitorScreenState extends State<AdminMonitorScreen> with TickerProviderStateMixin {
  ...
  List<Map<String, dynamic>> lockedStudents = [];
  bool isLoadingExercises = false;
  bool isLoadingActivities = false;
  bool isLoadingLockedStudents = false;
  ...
  void initState() {
    ...
    // Load locked students on initialization
    _loadLockedStudents();
  }
  ...
  void _setupSocketListeners() {
    ...
    // Listen for student lock/unlock events to refresh the locked list in real-time
    socketService.socket?.on('student-locked', (data) {
      debugPrint('Socket event: student-locked -> $data');
      _loadLockedStudents();
    });

    socketService.socket?.on('student-unlocked', (data) {
      debugPrint('Socket event: student-unlocked -> $data');
      _loadLockedStudents();
    });
  }

  Future<void> _loadLockedStudents() async {
    setState(() {
      isLoadingLockedStudents = true;
    });
    ...
    try {
      ...
      debugPrint('Loading locked students from API...');
      final locked = await apiService.getLockedStudents();
      debugPrint('Loaded locked students count: ${locked.length}');
      
      if (mounted) {
        setState(() {
          lockedStudents = locked;
          isLoadingLockedStudents = false;
        });
      }
    } catch (e) {
      print('Error loading locked students: $e');
      if (mounted) {
        setState(() {
          isLoadingLockedStudents = false;
        });
        _showErrorSnackBar('Failed to load locked students: $e');
      }
    }
  }

  Future<void> _unlockStudent(String enrollNumber, String studentName) async {
    try {
      final authService = Provider.of<AuthService>(context, listen: false);
      final apiService = ApiService(authService);
      
      await apiService.unlockStudent(enrollNumber);
      
      // Refresh locked students list
      await _loadLockedStudents();
      
      if (mounted) {
        _showSuccessSnackBar('$studentName has been unlocked successfully');
      }
    } catch (e) {
      print('Error unlocking student: $e');
      if (mounted) {
        _showErrorSnackBar('Failed to unlock student: $e');
      }
    }
  }
            ...
            child: Container(
              ...
              child: Column(
                ...
                children: [
                        ...
                        Expanded(
                          child: Column(
                            ...
                            children: [
                              ...
                              Text(
                                '${onlineStudents.length} online • ${lockedStudents.length} locked',
                                style: TextStyle(
                                  fontSize: 14,
                    ...
                    child: Row(
                      children: [
                        Container(
                          ...
                          child: const Icon(Icons.circle, color: Color(0xFF10B981), size: 12),
                        ),
                    ...
                    child: Row(
                      children: [
                        Container(
                          ...
                          child: const Icon(Icons.lock, color: Color(0xFFEF4444), size: 12),
                        ),
                        const SizedBox(width: 8),
                        Text(
                          'Locked Students (${lockedStudents.length})',
                          style: const TextStyle(
                            fontSize: 14,
                            fontWeight: FontWeight.w600,
                            color: Color(0xFF1E293B),
                        ...
                        IconButton(
                          ...
                          onPressed: isLoadingLockedStudents ? null : _loadLockedStudents,
                        ),
                      ],
                    ),
                  ),
                  ...
                  Expanded(
                    flex: 2,
                    child: isLoadingLockedStudents
                        ? const Center(
                            child: CircularProgressIndicator(
                              valueColor: AlwaysStoppedAnimation<Color>(Color(0xFF3B82F6)),
                            ),
                          )
                        : lockedStudents.isEmpty
                            ? const Center(
                                child: Padding(
                            ...
                            : ListView.builder(
                                padding: const EdgeInsets.all(12),
                                itemCount: lockedStudents.length,
                                itemBuilder: (context, index) {
                                  final student = lockedStudents[index];
                                  ...
                                  return Container(
                                    ...
                                    child: ListTile(
                                      ...
                                      trailing: ElevatedButton.icon(
                                        onPressed: () => _unlockStudent(student['enrollNumber'], student['name']),
