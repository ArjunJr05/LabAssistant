...
// Student self-lock endpoint (for tab switching detection)
router.post('/lock-self', auth, async (req, res) => {
  try {
    const userId = req.user.userId;
    const { tabSwitchCount, lockedAt } = req.body;
    
    console.log(`Student ${req.user.enrollNumber} self-locking due to ${tabSwitchCount} tab switches`);
    ...
    `, [tabSwitchCount || 2, lockedAt || new Date().toISOString(), userId]);
    
    if (result.rows.length > 0) {
      const student = result.rows[0];
      console.log(`✅ DATABASE UPDATE SUCCESSFUL: Student ${student.enroll_number} (${student.name}) locked successfully`);
      console.log(`🔒 User table updated: is_locked = true, tab_switch_count = ${tabSwitchCount || 2}`);
      ...
      if (io) {
        ...
        console.log(`📡 Socket event emitted: student-locked for ${student.enroll_number}`);
      }
      
      res.json({ 
        success: true, 
        message: 'Account locked successfully due to tab switching',
        student: student
      });
    } else {
      console.log(`❌ DATABASE UPDATE FAILED: No rows updated for user ID ${userId}`);
      res.status(500).json({ message: 'Failed to lock account' });
    }
  } catch (error) {
    console.error('Error in student self-lock:', error);
    res.status(500).json({ message: 'Server error', error: error.message });
  }
});

// Check if current student is locked
router.get('/check-lock-status', auth, async (req, res) => {
  try {
    const userId = req.user.userId;
    
    const result = await pool.query(`
      SELECT is_locked, tab_switch_count, locked_at
      FROM users 
