    ...
    class Program
    {
        ...
        static async Task StartAsync()
        {
            try
            {
                ...
                // Listen on all network interfaces (0.0.0.0) to accept remote connections
                _httpListener.Prefixes.Add($"http://+:{_port}/");
                _httpListener.Start();
                
                _isRunning = true;
                Console.WriteLine($"Screen Capture Agent started on port {_port}");
                Console.WriteLine($"Listening on all network interfaces (0.0.0.0:{_port})");
            ...
            try
            {
                ...
                if (File.Exists(configPath))
                {
                    ...
                    if (config?.port != null)
                    {
                        ...
                        Console.WriteLine($"Loaded port {port} from config.json");
                        return port;
                    }
                }
                else
