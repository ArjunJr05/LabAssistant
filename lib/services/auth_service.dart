...
class AuthService extends ChangeNotifier {
  ...
  Future<bool> login(String enrollNumber, String password) async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        final message = errorData['message'] ?? 'Your account has been locked';
        print('💥 Lock message: $message');
  ...
  Future<void> logout() async {
    try {
      ...
      if (wasAdmin) {
        print('🔑 Admin logout - Broadcasting shutdown to all students...');
        
        // Broadcast admin logout via Socket.IO to force all students to logout
        try {
          final response = await http.post(
            Uri.parse('${await ConfigService.getApiBaseUrl()}/api/admin/broadcast-logout'),
            headers: authHeaders,
            body: json.encode({
          ...
          print('📡 Admin logout broadcast sent: ${response.statusCode}');
        } catch (e) {
          print('❌ Failed to broadcast admin logout: $e');
        }
