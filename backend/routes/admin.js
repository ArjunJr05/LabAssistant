...
router.get('/student-exercises/:studentId', auth, adminOnly, async (req, res) => {
  try {
    ...
    const exercises = await pool.query(`
      SELECT 
        e.id, e.title, e.created_at, s.name as subject_name,
        CASE 
      ...
      GROUP BY e.id, e.title, e.created_at, s.name, sub.status, sub.score, sub.submitted_at
...
// Lock a student account due to malpractice
router.post('/lock-student', auth, adminOnly, async (req, res) => {
  try {
    const { enrollNumber, tabSwitchCount, lockedAt } = req.body;
    
    console.log(`Admin locking student: ${enrollNumber} with ${tabSwitchCount} tab switches`);
    
    if (!enrollNumber) {
      return res.status(400).json({ message: 'Enroll number is required' });
    }
    
    // Check if student exists
    const studentCheck = await pool.query('SELECT * FROM users WHERE enroll_number = $1 AND role = $2', [enrollNumber, 'student']);
    ...
    const result = await pool.query(`
      ...
      SET is_locked = true, 
          ...
          is_online = false,
          updated_at = NOW()
      WHERE enroll_number = $3 AND role = 'student'
      RETURNING *
    `, [tabSwitchCount || 2, lockedAt || new Date().toISOString(), enrollNumber]);
    
    if (result.rows.length > 0) {
      console.log(`Student ${enrollNumber} locked successfully`);
      
      // Emit socket event to notify admin dashboard
      const io = req.app.get('socketio') || req.app.get('io');
      if (io) {
        io.emit('student-locked', {
          enrollNumber: enrollNumber,
          name: student.name,
          tabSwitchCount: tabSwitchCount || 2,
...
router.get('/check-student-lock/:enrollNumber', auth, async (req, res) => {
  try {
    const { enrollNumber } = req.params;
    
    const result = await pool.query(`
      SELECT is_locked, tab_switch_count, locked_at, name
      FROM users 
    ...
    res.json({
      isLocked: student.is_locked || false,
      tabSwitchCount: student.tab_switch_count || 0,
      lockedAt: student.locked_at,
      name: student.name
    });
  } catch (error) {
    console.error('Error checking student lock status:', error);
    res.status(500).json({ message: 'Server error' });
  }
});

// Get all locked students
router.get('/locked-students', auth, adminOnly, async (req, res) => {
  try {
    console.log('Admin requesting locked students...');
    
    const result = await pool.query(`
      SELECT 
        id, name, enroll_number, year, section, batch,
        tab_switch_count, locked_at, is_locked
      FROM users 
      WHERE role = 'student' AND is_locked = true
      ORDER BY locked_at DESC
    `);
    
    console.log(`Found ${result.rows.length} locked students`);
    
    const lockedStudents = result.rows.map(student => ({
      id: student.id,
      name: student.name,
      enrollNumber: student.enroll_number,
      year: student.year,
...
router.post('/unlock-student', auth, adminOnly, async (req, res) => {
  try {
    const { enrollNumber } = req.body;
    
    console.log(`Admin unlocking student: ${enrollNumber}`);
    ...
    if (!student.is_locked) {
      return res.status(400).json({ message: 'Student is not locked' });
    }
    
    // Unlock student and reset tab switch count
    const result = await pool.query(`
      UPDATE users 
      SET is_locked = false, 
          tab_switch_count = 0, 
    ...
    if (result.rows.length > 0) {
      console.log(`Student ${enrollNumber} unlocked successfully`);
      
      // Emit socket event to notify admin dashboard
      const io = req.app.get('socketio') || req.app.get('io');
      if (io) {
        io.emit('student-unlocked', {
          enrollNumber: enrollNumber,
          name: student.name,
...
// Broadcast admin logout - forces all students to logout immediately
router.post('/broadcast-logout', auth, adminOnly, async (req, res) => {
  try {
    console.log(' ADMIN BROADCAST LOGOUT RECEIVED');
    
    const { message, timestamp } = req.body;
    
    // Get Socket.IO instance and broadcast admin shutdown
    const io = req.app.get('socketio') || req.app.get('io');
    if (io) {
      console.log('Broadcasting admin logout to all connected clients...');
      
      const shutdownData = {
        message: message || 'Admin has logged out. All students will be logged out.',
      ...
      // Emit to all connected sockets
      io.emit('admin-shutdown', shutdownData);
      console.log(`Admin logout broadcast sent to all connected clients`);
    }
    ...
    console.error('Error during admin logout broadcast:', error);
    res.status(500).json({ 
      success: false,
      message: 'Server error during logout broadcast',
      error: error.message 
    });
  }
});
